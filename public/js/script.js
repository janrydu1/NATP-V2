	...
	if($('#register-form').length){
		$('#register-form').submit(function(e){
			...
			var formFields = [
				...
				'#register-form #date',
				'#register-form #class',
				'#register-form input[name=accept_terms]'
			];
			...
			if (invalidField) {
			    var errorDiv = $('#register-form .response');
			    errorDiv.html('<div class="failed">Please fill the required fields.</div>');
			    $('html, body').animate({
			        scrollTop: errorDiv.offset().top - 130
			    }, 500);
            ...
            $.ajax({
                ...
                success:function(data){
                    $('form').trigger("reset");
                    $('#register-form .response').fadeIn().html(data);
                    setTimeout(function(){
                        $('#register-form .response').fadeOut("slow");
                    }, 5000);

	                var successDiv = $('#register-form .response');
				    $('html, body').animate({
				        scrollTop: successDiv.offset().top - 130
				    }, 500);
